...
def add():
    ...
    prompt = 'masukan nama buah: '
    nameFruit = pypi.inputStr(prompt=prompt, applyFunc = lambda x: x.capitalize(), blockRegexes=[r'[0-9]'])
    countFruit = pypi.inputInt(prompt='Masukkan jumlah buah: ', allowRegexes=[r'[0-9]'])
    priceFruit = pypi.inputInt(prompt='Masukkan hargabuah buah: ', allowRegexes=[r'[0-9]'])
    # Loop item di dalam listFruit
...
def delete():
    ...
    index = pypi.inputInt(prompt = 'Masukkan indeks buah yang ingin dihapus: ', allowRegexes=[r'[0-9]'])
...
"""
        ...
        menuNumber = int(input("Masukkan angka menu yang ingin dijalankan: "))
