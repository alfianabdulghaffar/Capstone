...
def search_android (database):
    # value = 'Android'
    scDataset = []
    for i, value in enumerate (database.value()):
        if value == 'Android':
            scDataset += i

    #     if value.upper() in list(database[i].values())[1] or value in list(database[i].values())[2]:
    #         scDataset.append(database[i])
    # if len(scDataset) > 0:
        show_data (scDataset)
...
def addFunction (database):
    ...
    if namePhone in list(database):
        ...
        # Input jenis RAM item, validasi dengan integer number
...
if __name__ == "__main__":
    ...
    path = r'C:\10. Project Capstone\phonemarketcapstone\data.csv'
